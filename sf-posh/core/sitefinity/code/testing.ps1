...
function sf-tests-runIntTests {
    ...
    sf-states-save -stateName init_tests

    $failedTests = _executeTestRunnerTests -testsNames $testsNames -categories $categories | ? {$_}
    if ($rerunFailed -and $failedTests) {
        sf-states-restore init_tests; sf-app-ensureRunning;
    ...
    sf-states-restore init_tests; sf-app-ensureRunning -InformationAction SilentlyContinue;
