$Script:stateCompleter = {
    ...
    $possibleValues = sf-states-get | select -ExpandProperty name
    if ($wordToComplete) {
...
Register-ArgumentCompleter -CommandName sf-states-restore -ParameterName stateName -ScriptBlock $Script:stateCompleter

function sf-states-remove {
    param(
...
Register-ArgumentCompleter -CommandName sf-states-remove -ParameterName name -ScriptBlock $Script:stateCompleter

# returns object with name and path
function sf-states-get {
    $statesPath = _getStatesPath
